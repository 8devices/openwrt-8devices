...
#define LIMA_WMAC_ART_OFFSET	0x1000
#define LIMA_ETH0_MAC_OFFSET	0x0
#define LIMA_ETH1_MAC_OFFSET	0x6
...
static struct gpio_keys_button lima_gpio_keys[] __initdata = {
	{
		.desc		= "wps button",
		.type		= EV_KEY,
		.code		= KEY_WPS_BUTTON, /* TODO: check code */ 
...
static void __init lima_setup(void)
{
	...
	ath79_register_usb();
	ath79_register_pci();
	ath79_register_wmac(art + LIMA_WMAC_ART_OFFSET, NULL);
	...
	ath79_init_mac(ath79_eth1_data.mac_addr, art + LIMA_ETH0_MAC_OFFSET, 0);
	...
	ath79_init_mac(ath79_eth0_data.mac_addr, art + LIMA_ETH1_MAC_OFFSET, 0);
