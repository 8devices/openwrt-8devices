...
#endif
    .btc_init_variables = rtl_btc_dummy,
	.btc_init_hal_vars = rtl_btc_dummy,
	.btc_init_hw_config = rtl_btc_dummy,
	.btc_ips_notify = rtl_btc_dummy,
	.btc_lps_notify = rtl_btc_dummy,
	.btc_scan_notify = rtl_btc_dummy,
	.btc_connect_notify = rtl_btc_dummy,
	.btc_mediastatus_notify = rtl_btc_dummy,
	.btc_periodical = rtl_btc_dummy,
	.btc_halt_notify = rtl_btc_dummy,
	.btc_btinfo_notify = rtl_btc_dummy,
	.btc_is_limited_dig = rtl_btc_dummy,
	.btc_is_disable_edca_turbo = rtl_btc_dummy,
	.btc_is_bt_disabled = rtl_btc_dummy,
	.btc_special_packet_notify = rtl_btc_dummy,
    .btc_set_hci_version = rtl_btc_dummy,
	.btc_set_bt_patch_version = rtl_btc_dummy,
	.btc_stack_update_profile_info = rtl_btc_dummy,
...
{
	rtl_btcoex_init_socket(priv);
}

void rtl_btc_close_socket(struct rtl8192cd_priv *priv)
{
	rtl_btcoex_close_socket(priv);
}
#if 0
...
void rtl_btc_init_hal_vars(struct rtl8192cd_priv *priv)
{
	...
	u8 bt_exist;
	u8 bt_type;
	ant_num = rtl_get_hwpg_ant_num(priv);
	RT_TRACE(COMP_INIT, DBG_DMESG,
		("%s, antNum is %d\n", __func__, ant_num));

	bt_exist = rtl_get_hwpg_bt_exist(priv);
	RT_TRACE(COMP_INIT, DBG_DMESG,
		("%s, bt_exist is %d\n", __func__, bt_exist));
	exhalbtc_set_bt_exist(bt_exist);

	bt_type = rtl_get_hwpg_bt_type(priv);
	RT_TRACE(COMP_INIT, DBG_DMESG,
		("%s, bt_type is %d\n", __func__, bt_type));
...
void rtl_btc_scan_notify(struct rtl8192cd_priv *priv, u8 scantype)
{
	...
	if(pBtMgnt->ExtConfig.bEnableWifiScanNotify)
		rtl_btcoex_SendScanNotify(priv, scantype);
...
void rtl_btc_btinfo_notify(struct rtl8192cd_priv *priv, u8 *tmp_buf, u8 length)
{
...
u8 rtl_get_hwpg_ant_num(struct rtl8192cd_priv *priv)
{
	...
	if (priv->pshare->btcoexist.btc_info.ant_num == ANT_X2)
		num = 2;
	else
		num = 1;
...
{
	struct rtl8192cd_priv *priv = rtl_priv(hw);
...
{
	return priv->pshare->btcoexist.btc_info.btcoexist;
}

u8 rtl_get_hwpg_bt_type(struct rtl8192cd_priv *priv)
{
	return priv->pshare->btcoexist.btc_info.bt_type;
}
