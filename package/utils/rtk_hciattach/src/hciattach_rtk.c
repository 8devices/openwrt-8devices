...
int get_part_by_name(const char * part_name, char * partition)
{
	char buf[128];
	char name[64];
	char part[64];
	...
	if (fp == NULL){
		RS_DBG("Failed to open /proc/mtd for read");
		return -1;
	}

	while (fgets(buf, sizeof(buf), fp))
	{
		if (sscanf(buf, "%64[^:]: %*x %*x \"%64[^\"]\"", part, name) == 2
			&& strcmp(name, part_name) == 0)
		{
			fclose(fp);
			sprintf(partition, "/dev/%s", part);
			return 0;
		}
	}
...
int rtk_get_bt_mac(char *mac)
{
	int bt_offset = 0x1000;
	int fd;
	unsigned char bt_sign[4] = {0x62, 0x74, 0x30, 0x31}; // bt01
	unsigned char hwpart[64];
	unsigned char buf[12];

	if(get_part_by_name("hwpart", hwpart)){
		RS_DBG("\"hwpart\" partition not found");
		return -1;
	}

	fd = open(hwpart, O_RDONLY);
	if(fd < 0 ){
		RS_DBG("Failed to open %s for reading", hwpart);
		return -1;
	}

	if(sizeof(buf) != pread(fd, buf, sizeof(buf), bt_offset)){
		RS_DBG("Failed to read hwpart (%s)", hwpart);
		close(fd);
		return -1;
	}

	if (memcmp(buf, bt_sign, 4)){
		RS_DBG("bt signature not found");
		close(fd);
		return -1;
	}
		...
		RT_U32 *config_baud_rate)
{
	...
	if(rtk_get_bt_mac(bt_addr))
		RS_INFO("Failed to read Bluetooth MAC address from flash, using default");

	RS_INFO("MAC Address: %02x:%02x:%02x:%02x:%02x:%02x",
