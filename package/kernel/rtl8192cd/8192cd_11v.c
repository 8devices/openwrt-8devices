		...
		if(!priv->pmib->wnmEntry.dot11vDiassocDeadline) {
			...
			for (i = 0; i < MAX_TRANS_LIST_NUM; i++) {
				...
				if(!pstat) {
					...
					// issue diassoc to trigger bss trans (802.11v)
					priv->del_sta_info.branch_code = 15;
					set_del_sta_info(priv, priv->bssTransPara.addr, _RSON_DISASSOC_DUE_BSS_TRANSITION);
