	...
	)
{
	...
	if(dm_value[0] == 100)
	{
		...
		PHYDM_SNPRINTF((output+used, out_len-used, "12. (( %s ))LNA_SAT_CHK_TYPE2\n", ((pDM_Odm->SupportAbility & ODM_BB_LNA_SAT_CHK_TYPE2)?("V"):(".")))); 
...
#if (PHYDM_LNA_SAT_CHK_TYPE2 == 1)
	phydm_lna_sat_type2_sm(pDM_Odm);
#endif
	odm_CCKPacketDetectionThresh(pDM_Odm);
	...
	)
{
	...
	switch	(CmnInfo)
	{
		...
		case	ODM_CMNINFO_MACID_RSSI_MIN:
			pDM_Odm->macid_rssi_min = (u1Byte)Value;
			break;
