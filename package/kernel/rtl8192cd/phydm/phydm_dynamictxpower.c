...
//#if ((RTL8192C_SUPPORT==1) || (RTL8192D_SUPPORT==1) || (RTL8188E_SUPPORT==1) || (RTL8812E_SUPPORT==1))
	...
	if ((priv->up_time % 3) == 0 )  {
		...
		{
			fa_cnt = pDM_Odm->FalseAlmCnt_Acc.Cnt_all_1sec[TDMA_DIG_HIGH_STATE];
			igi = ODMPTR->DM_DigTable.tdma_igi[TDMA_DIG_HIGH_STATE];			
		}
		else
		#endif
		{
			fa_cnt = pDM_Odm->FalseAlmCnt.Cnt_all;
			igi = (RTL_R8(0xc50) & 0x7f);
		}
		
		int disable_pwr_ctrl = ((fa_cnt > 1000 ) || ((fa_cnt > 300 ) && (igi >= 0x32))) ? 1 : 0;
