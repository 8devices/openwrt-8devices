	...
	)
{
	...
	if ((*pDM_Odm->pBandWidth == ODM_BW20M) && (((*pDM_Odm->pChannel >= 52) && (*pDM_Odm->pChannel <= 64)) || ((*pDM_Odm->pChannel >= 100) && (*pDM_Odm->pChannel <= 144))))
		ODM_SetBBReg(pDM_Odm, 0x838, 0xf0, 0x4);
...
)
{
	...
	if (idx <= 16) {
		...
		ODM_RT_TRACE(pDM_Odm, ODM_BB_DYNAMIC_PSDTOOL, ODM_DBG_LOUD, ("[Return Point] Idx Is Exceed, Not Support Dynamic Spur Detection and Eliminator\n"));
		return;
	}
...
BOOLEAN
config_phydm_switch_agc_tab_8822b(	
	IN	PDM_ODM_T				pDM_Odm,
	IN	u1Byte					central_ch,
	IN	bool					is_switch_sat_tab
	)
{
	...
	if (central_ch <= 14) {
		...
		if ((central_ch >= 36) && (central_ch <= 64)) {
			pDM_DigTable->agcTableIdx = is_switch_sat_tab ? 0x5 : 0x1;
		} else if ((central_ch >= 100) && (central_ch <= 144)) {
			pDM_DigTable->agcTableIdx = is_switch_sat_tab ? 0x6 : 0x2;
		} else if (central_ch >= 149) {
			pDM_DigTable->agcTableIdx = is_switch_sat_tab ? 0x7 : 0x3;
		} else {
			is_success = FALSE;
		}
	} else {
	...
	if (is_success)
		ODM_SetBBReg(pDM_Odm, 0x958, 0x1f, pDM_DigTable->agcTableIdx);
	else
		ODM_RT_TRACE(pDM_Odm, ODM_PHY_CONFIG, ODM_DBG_TRACE, ("%s: Fail to switch agc table (ch: %d)\n", __FUNCTION__, central_ch));
	...
	)
{
	...
	if(config_phydm_switch_agc_tab_8822b(pDM_Odm, central_ch, DEFAULT_AGC_TABLE) == FALSE)
	...
	if (central_ch <= 14) {
		...
		/* 2. AGC table selection, removed, use API */
