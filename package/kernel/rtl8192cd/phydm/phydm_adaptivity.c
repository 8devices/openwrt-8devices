...
)
{
	...
	if (pDM_Odm->SupportICType & (ODM_IC_11AC_GAIN_IDX_EDCCA | ODM_IC_11N_GAIN_IDX_EDCCA)) {
		...
		if (!pDM_Odm->Adaptivity_enable) {
			if (TH_L2H_dmc > L2H_TH_AbsUpperBound_mode2)
				TH_L2H_dmc = L2H_TH_AbsUpperBound_mode2;
			else if (TH_L2H_dmc < L2H_TH_AbsLowerBound_mode2)
				TH_L2H_dmc = L2H_TH_AbsLowerBound_mode2;
		}
		TH_H2L_dmc = TH_L2H_dmc - pDM_Odm->TH_EDCCA_HL_diff;
	}
#if (RTL8195A_SUPPORT == 0)
