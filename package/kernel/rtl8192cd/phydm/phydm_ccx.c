...
)
{
	...
	priv->pmib->miscEntry.nhm = 0;
	for (i = 1; i <= 11; i++){
		priv->pmib->miscEntry.nhm += (CCX_INFO->NHM_result[i] * CCX_INFO->NHM_th[i-1]);
		//GDEBUG("NHM_result[%d]=%d, NHM_th[%d]=%d\n", i, CCX_INFO->NHM_result[i], i-1, CCX_INFO->NHM_th[i-1] );
	}	
	priv->pmib->miscEntry.nhm = priv->pmib->miscEntry.nhm >> 8;
