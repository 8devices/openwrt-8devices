	...
	)
{
	...
	if((pEntry == NULL) || (pEntry->sta_in_firmware != 1))	
	{
		return;
	}
...
//			if(MimoPs <= MIMO_PS_DYNAMIC)
	...
	if (IS_BT_COEXIST_AVALIABLE(Adapter)) {
		u4Byte rate_bitmap = rtw_btcoex_GetRaMask(Adapter);
		ratr_bitmap &= ~rate_bitmap;
	}
...
#ifdef ENABLE_BT_COEXIST
	if (IS_ROOT_INTERFACE(Adapter) && Adapter->pmib->dot11OperationEntry.opmode&WIFI_STATION_STATE) {
		//if ((opmode & 0x01) == 0)
...
(
    ...
    u1Byte range3_en,
    u1Byte range4_en,
	u1Byte request_period
)
{
    u1Byte  H2CCommand[7] ={0};
    H2CCommand[0] =  (request_period <<5)| (range4_en<<4)| (range3_en<<3)| (range2_en<<2)| (range1_en<<1)| ATM_en;  
    FillH2CCmd88XX(Adapter, H2C_88XX_APREQ_ATM_RPT, 7, H2CCommand);
}
#endif
static VOID 
...
)
{
	...
	case C2H_88XX_EXTEND_IND:	
        ...
        #ifdef RTK_ATM
        else if(Extend_c2hSubID == EXTEND_C2H_88XX_ATM_RPT) 
        {    		
...
)
{
	...
	//printk("c2hCmdLen =%d \n",CmdLen);
    //printk("macid =%d \n",CmdBuf[6]);
    //printk("[7]=%x [8]=%x [9]=%x  \n",CmdBuf[7],CmdBuf[8],CmdBuf[9]);
    //printk("TX time = %x  \n",((CmdBuf[9]<<16)|(CmdBuf[8]<<8)|(CmdBuf[7])));
    
	for(i=0; i<NUM_STAT; i++) {
		if (priv->pshare->aidarray[i] && (priv->pshare->aidarray[i]->used == TRUE)){
			pstat = &(priv->pshare->aidarray[i]->station);
            j=6; //shift C2H ext-header
            while(pstat->aid != CmdBuf[j]){
                j+=4;  //find correspond macid
            }
            pstat->atm_C2H_tx_time= (CmdBuf[9]<<16)|(CmdBuf[8]<<8)|(CmdBuf[7]);
          }
    }
}
#endif
