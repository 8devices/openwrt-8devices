...
void rtl8192cd_dfs_det_chk(struct rtl8192cd_priv *priv)
{
	...
	CCA_count_cur = PHY_QueryBBReg(priv, 0xf08, 0xffff0000);
	...
	if (priv->pshare->rf_ft_var.dfs_det_print) {
		...
		// panic_printk("nhm_duty_cycle = %d\n", (100-((CCX_INFO->NHM_result_total * 100) >> 8)));
		...
		reg19e4_value = PHY_QueryBBReg(priv, 0x19e4, 0xffffffff);
...
*/
	priv->radar_det_mask_hist[priv->mask_idx] = 0;
        ...
        if ((pDM_Odm->SupportAbility & ODM_BB_NHM_CNT)){	
        	if (((CCX_INFO->NHM_result_total * 100) >> 8) >= priv->pshare->rf_ft_var.dfs_nhm_duty_cycle_th) {
			if (priv->pulse_flag_hist[index] == 1) {
				priv->pulse_flag_hist[index] = 0;
				if (priv->pshare->rf_ft_var.dfs_det_print4) {
	...
	if (PHY_QueryBBReg(priv, 0xf98, BIT(17)) || PHY_QueryBBReg(priv, 0xf98, BIT(19))){
		RTL_W32(0x924, RTL_R32(0x924) & ~BIT(15));
		RTL_W32(0x924, RTL_R32(0x924) | BIT(15));
	}
}
