						...
						if ( pstat && (pstat->IOTPeer == HT_IOT_PEER_BROADCOM)) {					
							...
							#endif
							{
								...
								else
								{
									...
									if(is_veriwave_testing(priv) && priv->pmib->dot11RFEntry.phyBandSelect ==  PHY_BAND_2G) {										
											txop = 0x300;
									}	
									else
#endif										
...
#define GET_ALL_STA_TX_AVERAGE(rtl8192cd_priv) (rtl8192cd_priv->ext_stats.tx_avarage>>17)
	...
	if (QOS_ENABLE) {
		if (!priv->pmib->dot11OperationEntry.wifi_specific 
			...
			if (priv->pshare->iot_mode_enable &&
				((priv->pshare->phw->VO_pkt_count > 120) ||
				 (priv->pshare->phw->VI_pkt_count > 120) ||
				 (priv->pshare->phw->BK_pkt_count > 120))) {
				priv->pshare->iot_mode_enable = 0;
				switch_turbo++;
...
//#ifdef CONFIG_WLAN_HAL_8881A
		...
		if ((OPMODE & WIFI_AP_STATE) && priv->pmib->dot11OperationEntry.wifi_specific)
		{
			if (!priv->pshare->iot_mode_VO_exist && (priv->pshare->phw->VO_pkt_count > 120)) {
	...
	if(AMPDU_ENABLE) {
        ...
        if(GET_ALL_STA_TX_AVERAGE(priv)>TP_HIGH_WATER_MARK) {
		...
		if (switch_turbo || priv->pshare->rf_ft_var.low_tp_txop) {
			...
			if (priv->pmib->dot11BssType.net_work_type & (WIRELESS_11N| WIRELESS_11G))
			{
				...
				{	
					...
					curr_tp = (unsigned int)GET_ALL_STA_TX_AVERAGE(tmppriv) + (unsigned int)(tmppriv->ext_stats.rx_avarage>>17);
				} 
				else 
#endif
				curr_tp = (unsigned int)GET_ALL_STA_TX_AVERAGE(priv) + (unsigned int)(priv->ext_stats.rx_avarage>>17);
	...
	if (switch_turbo && (GET_CHIP_VER(priv) == VERSION_8814A) ) {
		if (!priv->pshare->iot_mode_enable && !priv->pshare->aggrmax_change) {
			...
			RTL_W16(REG_BIT_MAX_AGG_NUM, priv->pshare->aggrmax_bak);
			priv->pshare->aggrmax_change = 0;
		}
	} 
#endif
#ifdef TX_EARLY_MODE
		unsigned int em_tp = (GET_ALL_STA_TX_AVERAGE(priv) + (priv->ext_stats.rx_avarage>>17));
		if (em_tp > 80)
